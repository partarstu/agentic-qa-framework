...
2. **Duplicate Detection (RAG)**:
    * Search RAG with Jira issues (bugs) for existing bugs matching the failure. Use the pydantic ai example for
      qdrant in order to correctly implement the corresponding agent search
...
RAG is to be implemented using pydantic-ai with qdrant. All the corresponding config must be present.
