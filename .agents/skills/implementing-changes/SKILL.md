...
1. The input is an implementation plan, a change request or a request for a bug fix. Without a plan the user has approved, write one with the `architecting-features` skill and get the user's approval. The approval covers the whole loop, including the fixes the review and test phases request. A plan whose *Architecture* section names a CALM change but lacks the line `CALM updated, validated and approved by the user on <date>` is refused: send it back to the `architecting-features` skill (*Architecture first* in `AGENTS.md`) and do not start.
