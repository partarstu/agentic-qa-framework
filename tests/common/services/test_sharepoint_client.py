...
from common.services.sharepoint_client import GRAPH_SCOPE, DeltaResyncRequired, SharePointClient
...
def test_an_expired_delta_link_requires_a_full_resync(token_endpoint):
    with _serve(lambda request: httpx.Response(410)), pytest.raises(DeltaResyncRequired):
        SharePointClient().enumerate_delta(DRIVE_ID, "https://graph/expired-delta")
