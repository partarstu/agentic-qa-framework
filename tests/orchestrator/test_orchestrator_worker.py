...
from orchestrator.main import _agent_worker, _execute_single_test, AgentStatus
from a2a.types import TaskState, TaskStatus, Artifact, TextPart
...
async def test_agent_worker_success(mock_registry, mock_queue):
    ...
    with patch("orchestrator.main._execute_single_test", new_callable=AsyncMock) as mock_exec:
        ...
        results = []
        try:
            await _agent_worker("agent-1", mock_queue, results)
